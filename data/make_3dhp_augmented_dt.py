        ...
        for z_trans in z_trans_candi:
            for phi_rot in gp_phi_rot_candi:
                for p_trans in person_trans_candi:
                    p_trans_ub = p_trans * 3000 #6000
                    p_trans_lb = p_trans * -3000 #-6000
                    ...
                    for c in range(num_cam):
                        ...
                        while (s < seq_per_comb):
                            subjects = np.random.choice(num_subjects, size=int(num_people * 1.5), replace=False)
                            seqs = np.random.choice(2, size=int(num_people * 1.5), replace=True)
                            ...
                            while flag:
                                ...
                                if len(_kp3ds) != int(num_people * 1.5):
                                    print('[Z] FAIL!')
                                    continue

                                kp3ds_sel = np.stack(_kp3ds, axis=1)


                                ## z-direction GP translation
                                z_translation = z_trans*np.array([[0., 0., 1.]])
                                kp3ds_sel += z_translation[None,:,None,:]


                                joints_3d, roots = [], []
                                i, idx = 0, 0
                                while i < max_iter and (num_people - len(joints_3d) <= int(num_people * 1.5) - idx):
                                    ## Make a valid augmentation (Roots are all visible throught sequence)
                                    kp3d = kp3ds_sel[:,idx:(idx+1)].copy()
                                    Xs = kp3d[:,:,root_id,0]
                                    Ys = kp3d[:,:,root_id,1]
                                    Zs = kp3d[:,:,root_id,2]
                                    ...
                                    if kv_min_res.success and kv_max_res.success and kv_min_res.fun <= -kv_max_res.fun:
                                        ...
                                        kv = np.clip(np.random.uniform(kv_min, kv_max, size=(1,1)), kv_min, kv_max)
                                    ...
                                    if ku_min_res.success and ku_max_res.success and ku_min_res.fun <= -ku_max_res.fun:
                                        ...
                                        ku = np.clip(np.random.uniform(ku_min, ku_max, size=(1,1)), ku_min, ku_max)
                                    ...
                                    kp3d += u_translation[None,:,None,:]

                                    ##
                                    if len(roots)==0 or np.linalg.norm(kp3d[:,:,root_id,:2] - np.stack(roots, axis=1), axis=-1).min() > dist_thre:
                                        roots.append(kp3d[:,0,root_id,:2])
                                        joints_3d.append(kp3d[:,0])
                                        idx += 1
                                        if len(joints_3d) == num_people:
                                            break
                            ...
                            ##
                            kp3d = np.stack(joints_3d, axis=1)
                    ...
                    np.savez_compressed(f'data/data_3d_vid3dhp_{augtype}_img_1_3M_230227_Abl_GP_{"Y" if gp_aware else "N"}_OCC_{"Y" if occ_aware else "N"}.npz', positions_3d=dataset_3d)
                    np.savez_compressed(f'data/data_2d_vid3dhp_{augtype}_img_1_3M_230227_Abl_GP_{"Y" if gp_aware else "N"}_OCC_{"Y" if occ_aware else "N"}.npz', positions_2d=dataset_2d, vis=dataset_vis, obj=dataset_obj, img_path=dataset_img_path)
